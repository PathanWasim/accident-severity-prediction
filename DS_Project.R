...
predict_accident_severity <- function(input_data, model, dummies, classes) {
  # Convert input data to appropriate format
  # Ensure all required features are present in newdata
  missing_cols <- setdiff(colnames(train_data), colnames(input_data))
  for (col in missing_cols) {
    if (is.numeric(train_data[[col]])) {
      input_data[[col]] <- 0  # Assign 0 for missing numeric features
    } else {
      input_data[[col]] <- "Unknown"  # Assign "Unknown" for missing categorical features
    }
  }
  
  
  # One-hot encode features
  features_encoded <- predict(dummies, input_data)
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    file_path <- "D:\\VIT Notes\\VIT SEM 4\\Data Science\\CP\\Adesh r project\\road_accident_dataset.csv"
  ...
  output$model_accuracy_box <- renderValueBox({
    if (!is.null(rv$evaluation)) {
      accuracy <- round(rv$evaluation$accuracy * 100, 2)
      valueBox(
        paste0(accuracy, "%"),
        "Model Accuracy",
        icon = icon("chart-pie"),
  ...
  output$model_metrics_table <- renderTable({
    ...
    metrics <- data.frame(
      ...
      Value = c(
        rv$evaluation$confusion_matrix$overall["Accuracy"],
  ...
  prediction_result <- eventReactive(input$predict_button, {
    ...
    # Make prediction
    predict_accident_severity(input_data, rv$model, rv$dummies, rv$classes)
  })
